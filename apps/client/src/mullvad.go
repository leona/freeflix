...
func (m *Mullvad) SetKeyPair() {
	...
	if _, err := os.Stat("/app/config/client/.mullvad.keypair"); errors.Is(err, os.ErrNotExist) {
		...
		file, err := os.Create("/app/config/client/.mullvad.keypair")
	...
	file, err := os.Open("/app/config/client/.mullvad.keypair")
	...
	for _, country := range m.Servers.Countries {
		...
		for _, city := range country.Cities {
			for _, relay := range city.Relays {
				configPath := "/app/config/client/mullvad-" + relay.Hostname + ".conf"
